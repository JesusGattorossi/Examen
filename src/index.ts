let vendedorNombres: string[]= [ "Camila",  "Franco", "Sofia", "Matias","Agustina"];
...
function buscarVentaMaxima(ventasVendedor:number[],numeroVendedor: number) {
  let ventaMaxima = 0;
  let indiceVentaMAxima=0;
  for (let i=0;i < ventasVendedor.length; i++){
    if (ventasVendedor[i] >ventaMaxima) {
      ventasMaxima= ventasVendedor[i];
      indiceVentaMaxima= i;

    }
  }
  console.log('las ventas del vendedor ${vendedorNombres[numeroVendedor]}, es': );
}